function [im_recon,a] = SubspaceRecon_Process_BP_v2(kdata,sens_map,P_dB,Phase0_all,TEs,Phi,parameters,scaling,bart_savepath,recon_algorithm,GPU_flag)
...
if recon_algorithm.regularization == 1
    Rec=bart(['picsS ',gpu,' -S -d 0 -i 100 -u ',num2str(recon_algorithm.rho),' -C 8 -m -w ',num2str(scaling),' -b ',num2str(recon_algorithm.block_size),' -R L:7:7:',num2str(recon_algorithm.lambda),...
...
if(PF_shift>0 && (recon_algorithm.PF_recon == 1) )
    im_recon = ifft2c(mrir_filter_raw_apodize_1d(fft2c(im_recon), 2, 0.1)) ;
end
