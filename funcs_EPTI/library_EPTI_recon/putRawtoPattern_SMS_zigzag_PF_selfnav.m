function [kdata] = putRawtoPattern_SMS_zigzag_PF_selfnav(kdata,Nseg,Rseg,Rpe,MB,PFshift)
...
if MB>1
    ...
    kdata(1:2,:,:,1,:,:)=kdata_EPTI(1:2,:,:,:,:);
    for i = 1:MB
        kdata(2+i:MB:end,:,:,i,:,:)=kdata_EPTI(2+i:MB:end,:,:,:,:);
    end
%     for i = 1:MB
