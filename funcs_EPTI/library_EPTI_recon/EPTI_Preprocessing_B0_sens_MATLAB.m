function [sens_map_all,P_dB_all,Phase0_odd_all,Phase0_even_all,gccmtx_aligned] = EPTI_Preprocessing_B0_sens_MATLAB(directory,filename,kdata_calib0,parameters,parameters_calib,save_data_flag) 
...
% This version uses MATLAB for sensitivity estimation
...
target_size = [nx,npe];
for slice=1:N_slice
    fprintf('calculating sensitivity slice#: %d/%d \n',slice,N_slice);
    kdata_calib=crop( squeeze(fft2c(cat(4, 1e-6 * volume_svd(:,:,slice), im_echo1(:,:,slice,:)))),[num_acs,num_acs,ncc+1]);
    sens = Espirit_smap( kdata_calib,target_size,c );
    sens=sens(:,:,2:end);
    sens_map(:,:,slice,:) = sens;   
end
