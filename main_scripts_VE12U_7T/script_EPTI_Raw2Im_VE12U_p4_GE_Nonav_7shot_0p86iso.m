...
% This script has specific modifications for VE12U EPTI sequence without
% navigator acquisition for early TEs, including self-B0 updating.
...
filename_calib = fullfile(directory,'meas_MID00892_FID119693_EPTI_p4_calibration_0p86iso.dat');     % path for rawdata file of the calibration scan
filename_EPTI = fullfile(directory,'meas_MID00894_FID119695_EPTI_p4_Nonav_7shot_GE_0p86iso.dat');   % path for rawdata file of the imaging scan
save_filename = 'EPTI_p4_7shot_GE_0p86iso';                                                         % filename for output data
...
process_type = 0;       % '0': process GE EPTI data;      '1': process SE EPTI data
...
if recon_algorithm.regularization == 1
    ...
    recon_algorithm.lambda = 2e-3;          % lambda for regularization
    recon_algorithm.block_size = 12;        % block size for LLR
end
...
recon_algorithm.apodization = 0;            % perform apodization to reduce Gibbs artifact. '1': yes; '0': no
recon_algorithm.apodize_factor = 0.1;       % apodization factor
recon_algorithm.B0update = 2;               % '0': no self B0 update; '1': global (0 order) B0 update; '2': 2D B0 update (stable when acceleration is low, but may still reduce tSNR)
...
% end
Time_varing_phasecor = 0;                   % turned off for this sequence without navigator
StartEcho_forPhasecor = 6;                  % start from which echo in calibration data to calculate odd-even echo phase difference for all echoes (skip early echoes which may have strong variation at 7T)
...
switch process_type  
    case 0
        ...
        if recon_algorithm.B0update ~= 0
            [Phi_B0,~,S_B0] = EPTI_gen_Subspace_Basis_GE_B0(meas.prot,K+2);
            scaling_for_Phi_B0 = (S_B0(1:K+2).^(1/3))';
            Phi_B0 = Phi_B0.*scaling_for_Phi_B0;
        end
    case 1
...
for dif_dyn=dyn_to_recon
    ...
    [kdata,~] = ReadRaw_EPTI_image(filename_EPTI,meas.prot,dif_dyn,process_type,seq_type,gccmtx_aligned,linear_fit_coeff);
    ...
    for dif_slice=slice_to_recon            % slice loop
        ...
        [im_recon,a,P_dB_updated] = SubspaceRecon_Process_B0update(kdata_tmp,sens_map,P_dB,...
            Phase0_use_t,TEs_all,Phi,Phi_B0,meas.prot,scaling,BART_savepath,recon_algorithm,GPU_flag); 
        ...
        if show_image_flag == 1
            ...
            figure; imshow3(im_recon_show(end:-1:1,:,:),[0 25],[1,meas.prot.NslicesEX]);
        end
        im_epti(:,:,dif_slice+(0:meas.prot.NslicesEX-1)*meas.prot.N_slice,:)=im_recon(:,:,end:-1:1,:);
        P_dB_all(:,:,slice_group) = P_dB_updated;
        %%
    end
