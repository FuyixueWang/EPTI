...
static void process_option2(char c, const char* optarg, const char* name, const char* usage_str, const char* help_str, int n, const struct opt_s opts[n ?: 1])
{
	if ('h' == c) {
...
void cmdline2(int* argcp, char* argv[], int min_args, int max_args, const char* usage_str, const char* help_str, int n, const struct opt_s opts[n ?: 1])
{
	int argc = *argcp;
	char optstr[2 * n + 2];
	...
	out:	continue;
#else
	process_option2(c, optarg, argv[0], usage_str, help_str, n, opts);
#endif
