...
static struct lrthresh_data_s* lrthresh_create_data2(const long dims_decom[DIMS], bool randshift, unsigned long mflags, const long blkdims[MAX_LEV][DIMS],
...
{
	struct lrthresh_data_s* data = lrthresh_create_data2(dims_lev, randshift, mflags, blkdims, lambda, noise, remove_mean, overlapping_blocks,option);
...
long llr_blkdims(long blkdims[MAX_LEV][DIMS], unsigned long flags, const long idims[DIMS], long llrblk)
{
    ...
    blkdims[0][2] = 1; // Zijing, hard code to to avoid LLR along SMS slices (kz)
	return 1;
}
