...

{
	...
	for (int nr = 0; nr < ropts->r; nr++) {
		...
		case L1WAV:
			...
			if(nr>=getLinopOutCounter()-1) {   // add
				trafos[nr] = linop_identity_create(DIMS, img_dims);
			} else {
				debug_printf(DP_INFO, "Using linopScript\n");
				trafos[nr] = getLinopsOutVec()[nr+1];
			}
			prox_ops[nr] = prox_wavelet_thresh_create(DIMS, linop_codomain(trafos[nr])->dims, wflags, regs[nr].jflags,WAVELET_DAU2, minsize,
		...
		case TV:
			...
			if(nr<getLinopOutCounter()-1) {
				const struct linop_s* tmp=linop_grad_create(linop_codomain(getLinopsOutVec()[nr+1])->N, linop_codomain(getLinopsOutVec()[nr+1])->dims,linop_codomain(getLinopsOutVec()[nr+1])->N, regs[nr].xflags);
		...
		case LLR:
			...
			debug_printf(DP_WARN, "lowrank regularization: %f\n", regs[nr].lambda);
