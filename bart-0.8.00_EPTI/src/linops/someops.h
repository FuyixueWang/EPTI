...
extern struct linop_s* linop_matrix_chain(const struct linop_s* a, const struct linop_s* b); //add

// extern struct linop_s* linop_resizeBase_create(unsigned int N, const long out_dims[N], const long in_dims[N]); //add

extern struct linop_s* linop_print_create(unsigned int N, const long dims[__VLA(N)], long messageId); //add
