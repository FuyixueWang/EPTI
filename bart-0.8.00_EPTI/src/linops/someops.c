...
static void resizeBase_forward(const linop_data_t* _data, complex float* dst, const complex float* src) //add
{
	const auto data = CAST_DOWN(resize_op_s, _data);
...
static void resizeBase_adjoint(const linop_data_t* _data, complex float* dst, const complex float* src)  //add
{
	const auto data = CAST_DOWN(resize_op_s, _data);
...
static void resizeBase_normal(const linop_data_t* _data, complex float* dst, const complex float* src)  //add
{
	const auto data = CAST_DOWN(resize_op_s, _data);
...
struct linop_s* linop_resizeBase_create(unsigned int N, const long out_dims[N], const long in_dims[N]) //add
{
	PTR_ALLOC(struct resize_op_s, data);
	SET_TYPEID(resize_op_s, data);
...
static DEF_TYPEID(transpose_op_s_2);
...
{
	PTR_ALLOC(struct transpose_op_s_2, data);
	SET_TYPEID(transpose_op_s_2, data);
