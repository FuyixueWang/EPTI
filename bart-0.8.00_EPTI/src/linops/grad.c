...
static void grad_dims_use(unsigned int D, long dims2[D + 1], unsigned int flags, const long dims[D]) // add
{
...
struct linop_s* linop_gradm_create(long N, const long dims[N], unsigned int flags)  // add
{
	PTR_ALLOC(struct grad_s, data);
	SET_TYPEID(grad_s, data);

	long dims2[N ];
	grad_dims_use(N-1, dims2, flags, dims);
