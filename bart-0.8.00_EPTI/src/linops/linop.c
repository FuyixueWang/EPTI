...
struct linop_s* linop_chainAdj(const struct linop_s* a, const struct linop_s* b)  //add
{
	PTR_ALLOC(struct linop_s, c);

	c->forward = operator_chain(a->forward, b->adjoint);
	c->adjoint = operator_chain(b->forward, a->adjoint);
...
struct linop_s* linop_chainNormal(const struct linop_s* a, const struct linop_s* b)  //add
{
	PTR_ALLOC(struct linop_s, c);

	c->forward = operator_chain(a->forward, b->normal);
	c->adjoint = operator_chain(b->normal, a->forward);
...
struct linop_s* linop_PutFowrardOfBInNormalOfA(const struct linop_s* a, const struct linop_s* b) //add
{
	PTR_ALLOC(struct linop_s, lo);

	lo->forward = operator_ref(a->forward);
	lo->adjoint = operator_ref(a->adjoint);
	lo->normal = operator_ref(b->forward);
	lo->norm_inv = operator_p_ref(a->norm_inv);
